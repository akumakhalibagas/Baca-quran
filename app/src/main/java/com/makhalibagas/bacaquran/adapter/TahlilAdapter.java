...
public class TahlilAdapter extends RecyclerView.Adapter<TahlilAdapter.DoaViewHolder> {

    private Context context;
    private List<Tahlil> tahlilList;

    public TahlilAdapter(Context context, List<Tahlil> tahlilList) {
    ...
    @NonNull
    @Override
    public TahlilAdapter.DoaViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.item_tahlil, parent, false);
        return new DoaViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull TahlilAdapter.DoaViewHolder holder, final int position) {

        final Tahlil doa = tahlilList.get(position);
    ...
    public int getItemCount() {
        return tahlilList == null ? 0 : tahlilList.size();
    }
